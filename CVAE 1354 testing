...
GEN_DATA_FILE_DIR = r"C:\gen_data\1354bus_data.csv"
...
class Encoder(nn.Module):
    def __init__(self, target_dim, condition_dim, latent_dim):
        ...
        self.fc_mu = nn.Linear(HIDDEN_6, latent_dim)
        self.fc_logvar = nn.Linear(HIDDEN_6, latent_dim)
...
class Decoder(nn.Module):
    def __init__(self, latent_dim, condition_dim, target_dim):
        ...
        self.fc1 = nn.Linear(latent_dim + condition_dim, HIDDEN_6)
        self.fc2 = nn.Linear(HIDDEN_6, HIDDEN_5)
        self.fc3 = nn.Linear(HIDDEN_5, HIDDEN_4)
        self.fc4 = nn.Linear(HIDDEN_4, HIDDEN_3)
        self.fc5 = nn.Linear(HIDDEN_3, HIDDEN_2)
        self.fc6 = nn.Linear(HIDDEN_2, HIDDEN_1)
...
if __name__ == "__main__":
    ...
    for case in tqdm(range(num_test_samples), desc="Testing IEEE1354-bus with AC-OPF FLAT-START"):
    ...
    for case in tqdm(range(num_test_samples), desc="Testing IEEE1354-bus with AC-OPF WARM-START"):
        ...
        p_ext_grid = x_pred_raw[0][2708:2709]
        q_ext_grid = x_pred_raw[0][2709:2710]

        p_gen = x_pred_raw[0][2710:3228:2]
        q_gen = x_pred_raw[0][2711:3228:2]

        assert len(vm_pu) == len(net.bus)
        assert len(va_degree) == len(net.bus)
        assert len(p_ext_grid) == len(net.ext_grid)
        assert len(q_ext_grid) == len(net.ext_grid)
        assert len(p_gen) == len(net.gen)
        assert len(q_gen) == len(net.gen)
    ...
    for case in tqdm(range(num_test_samples), desc="Testing IEEE1354-bus with DC-OPF"):
