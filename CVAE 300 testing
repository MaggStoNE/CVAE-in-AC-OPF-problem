...
class Encoder(nn.Module):
    def __init__(self, target_dim, condition_dim, latent_dim):
        ...
        self.fc_mu = nn.Linear(HIDDEN_5, latent_dim)
        self.fc_logvar = nn.Linear(HIDDEN_5, latent_dim)
...
class Decoder(nn.Module):
    def __init__(self, latent_dim, condition_dim, target_dim):
        ...
        self.fc1 = nn.Linear(latent_dim + condition_dim, HIDDEN_5)
        self.fc2 = nn.Linear(HIDDEN_5, HIDDEN_4)
        self.fc3 = nn.Linear(HIDDEN_4, HIDDEN_3)
        self.fc4 = nn.Linear(HIDDEN_3, HIDDEN_2)
        self.fc5 = nn.Linear(HIDDEN_2, HIDDEN_1)
...
if __name__ == "__main__":
    ...
    for case in tqdm(range(num_test_samples), desc="Testing IEEE300-bus with AC-OPF FLAT-START"):
    ...
    for case in tqdm(range(num_test_samples), desc="Testing IEEE300-bus with AC-OPF WARM-START"):
        ...
        p_ext_grid = x_pred_raw[0][600:601]
        q_ext_grid = x_pred_raw[0][601:602]

        p_gen = x_pred_raw[0][602:738:2]
        q_gen = x_pred_raw[0][603:738:2]
    ...
    for case in tqdm(range(num_test_samples), desc="Testing IEEE300-bus with DC-OPF"):
