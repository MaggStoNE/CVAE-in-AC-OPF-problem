...
class Decoder(nn.Module):
    def __init__(self, latent_dim, condition_dim, target_dim):
        ...
        self.fc1 = nn.Linear(latent_dim + condition_dim, HIDDEN_2)
        self.fc2 = nn.Linear(HIDDEN_2, HIDDEN_1)
        self.fc3 = nn.Linear(HIDDEN_1, target_dim)
...
if __name__ == "__main__":
    ...
    for case in tqdm(range(num_test_samples), desc="Testing IEEE14-bus with AC-OPF FLAT-START"):
    ...
    for case in tqdm(range(len(c_test_raw)), desc="Testing IEEE14-bus with AC-OPF WARM-START"):
        ...
        p_ext_grid = x_pred_raw[0][28:29]
        q_ext_grid = x_pred_raw[0][29:30]

        p_gen = x_pred_raw[0][30:38:2]
        q_gen = x_pred_raw[0][31:38:2]
    ...
    for case in tqdm(range(num_test_samples), desc="Testing IEEE14-bus with DC-OPF"):
